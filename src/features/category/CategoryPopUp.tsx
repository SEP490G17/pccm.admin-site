...
const CategoryPopUp: React.FC<CategoryPopUpProps> = observer(({ isOpen, onClose }) => {
  ...
  const handleAddCategory = async () => {
    ...
    await addCategory({ categoryName: newCategoryName }, toast);
    setNewCategoryName('');
  };

  const handleEditCategory = async (id: number, newName: string) => {
    if (newName.trim() === '') return;
    await updateCategory({ id, categoryName: newName }, toast);
  };

  const handleDeleteCategory = async (id: number) => {
    await deleteCategory(id, toast);
  };
