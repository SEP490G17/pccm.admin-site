...
const ServicePage = observer(() => {
    ...
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        serviceStore.setSearchTerm(e.target.value);
    };
    ...
    const handlePageSizeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setPageSize(newSize);
        mockLoadServices(); // Gọi lại hàm để tải dữ liệu với số lượng mới
    };

    const renderPaginationButtons = () => {
        const { pageIndex, totalPages } = servicePageParams;
        const buttons = [];
    ...
    return (
        <Flex direction="column" p={8} bg="#F4F4F4">
            <PageHeadingAtoms title={'Danh sách dịch vụ'} />
            <Flex width="100%" justifyContent="space-between" alignItems="flex-end" mb="1.5rem">
                <Flex gap="30px" alignItems="center">
                    ...
                    <Button colorScheme="teal" size="md" leftIcon={<FaEdit />} width="149px" height="35px" background="#FFF" color="black" border="1px solid #ADADAD" onClick={() => router.navigate('/dich-vu/tao')}>
                        Thêm mới
                    </Button>
                </Flex>
            ...
            <TableContainer bg={'white'} borderRadius={'8px'} padding={0} border={'1px solid #000'} mb="1.5rem">
                <Table variant="simple" cellPadding={'1rem'} padding={0}>
                    ...
                    <Tbody>
                        {loading ? (
                            <SkeletonTableAtoms numOfColumn={6} pageSize={servicePageParams.pageSize} />
                        ) : (
                            serviceArray.map((service, index) => (
                                <Tr key={service.id}>
                                    <Td borderBottom={'0.923px solid #BDBDBD'} borderRight={'0.923px solid #BDBDBD'}>
                                        {(servicePageParams.pageIndex - 1) * servicePageParams.pageSize + index + 1}
                                    </Td>
                                    <Td borderBottom={'0.923px solid #BDBDBD'} borderRight={'0.923px solid #BDBDBD'}>
                                        {service.name}
                                    </Td>
            ...
            <Flex justifyContent="space-between" alignItems="center" mb="1rem">
                ...
                <Box display="flex" alignItems="center">
                    ...
                    <Select
                        width="70px"
                        height="35px"
                        value={servicePageParams.pageSize}
