...
const BookingConflictGridItem: FC<BookingGridItemProps> = ({ bookingToday, index }) => {
  ...
  const { denySingleConflictBooking } = bookingClusterStore;
  const toast = useToast();
  const handleDenyBooking = async (id: number) => {
    await denySingleConflictBooking(id, toast).then((data) => {
      if (data.res) {
      ...
      <GridItem colSpan={22} as={Link} to={`/booking/chi-tiet/${bookingToday.id}`}>
        <Center className="h-full">
          <Grid templateColumns={'repeat(23, 1fr)'} className="w-full">
            ...
            <GridItem colSpan={3}>{dayjs(bookingToday.startDay, 'DD/MM/YYYY').format('DD/MM/YYYY')}</GridItem>
            <GridItem colSpan={3}>{dayjs(bookingToday.endDay, 'DD/MM/YYYY').format('DD/MM/YYYY')}</GridItem>
            <GridItem colSpan={2}>
